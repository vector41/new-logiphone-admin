...
import paginator from "@/presets/lara/paginator";
...
const gettingEmpDatas = async (_page) => {
    ...
    let res_emp = await getPhoneBookList(
        _page,
        filter_store_position.value,
        filter_typeCategories.value,
        filter_keyword.value,
                    ...
                    <div class="w-full h-full flex flex-col m-2 py-4 mx-auto">
                        <div class="flex items-center justify-between pt-4">
                            <div class="flex items-center gap-4">
                                <Dropdown v-model="store_position" :options="store_postions" option-label="name"
                                    placeholder="全て表示" class="w-52" />
                                <InputText type="text" class="w-64" v-model="keyword" placeholder="キーワード 例: ドラボックス" />
                                <div class="flex items-center justify-center gap-1">
                                    <Dropdown v-model="pref" :options="prefs" option-label="value" placeholder="都道府県"
                                        class="w-36" />
                                    <InputText type="text" class="w-60" v-model="address" placeholder="住所" />
                                </div>
                                <Button icon="pi pi-search" label="検索" class="h-[34px] text-white"
                                    @click="filterEmployeers" />
                            </div>
                            <div class="flex items-center gap-4">
                                <Button icon="pi pi-ellipsis-h" severity="info" label="詳細" class="h-[34px] text-white"
                                    @click="detailEmployeer" />
                                <Button icon="pi pi-trash" severity="danger" label="削除" class="h-[34px] text-white"
                                    @click="removeEmployeer" />
                            </div>
                        </div>
                        <Paginator class="pt-2 main-paginator" :rows="teble_rows" :totalRecords="totalCount" :first="1"
                            @page="handlePageChange">
                            <template #start="slotProps">
                                <div class="flex justify-center gap-3">
                                    <label class="font-bold">表示順:
                                        <span class="text-green-500">{{ slotProps.state.rows }}</span>
                                        件</label>
                                    <label class="font-bold">検索結果: <span class="text-green-500">{{ totalCount }}</span>
                                        件
                                    </label>
                                    <label class="font-bold">ページ:
                        ...
                        <div
                            ...
                            <DataTable v-show="loading == false" :value="filtered_employeers" stripedRows
                                :rows="teble_rows" :class="`p-datatable-sm`" tableStyle="min-width: 50rem; "
                                style="width: -webkit-fill-available">
                                <Column field="" header="" class="col-data">
                                    <template #body="{ data }">
                                        <Checkbox name="remember" class="text-center w-auto" v-model="data.checked"
                                            binary />
                                    </template>
                                </Column>
                                <Column field="" header="" class="col-data">
                                    <template #body="{ data }">
                                        <i v-if="data.store_pos == 2"
                                ...
                                <Column field="" header="役割" class="col-data">
                                    <template #body="{ data }">
                                        <Tag v-for="(role, index) in data.roles" :key="index" :value="role.charAt(0)"
                                            rounded severity="info"></Tag>
                        ...
                        <Dialog v-model:visible="show_detail_dialog" :header="employee_title"
                            :style="{ width: '1250px' }" position="center" :modal="true" :draggable="false"
                            :pt="{ root: 'border-none', content: { style: 'overflow:unset' } }">
                            <EmployeerComponent :type="`update`" :employeer_obj="selected_employee"
                                @change_employeer="changeEmployeerData" />
                            <div class="flex justify-center gap-2">
                                <Button type="button" label="キャンセル" class="w-32 h-8" severity="secondary" outlined
                                    @click="show_detail_dialog = false"></Button>
                                <Button type="button" label="更 新" class="text-white w-32 h-8"
                                    @click="updateEmployeer"></Button>
                            </div>
                        </Dialog>

                        <div id="sms_send_dialog">
                            <Dialog v-model:visible="show_sms_dialog" :header="sms_dialog_title"
                                :style="{ width: '1250px' }" position="center" :modal="true" :draggable="false"
                                :pt="{ root: 'border-none', content: { style: 'overflow:unset' } }">
                                ...
                                <div class="flex justify-center gap-2 mt-5">
                                    <Button type="button" label="アプリに転送" class="w-64 h-8" severity="secondary" outlined
                                        @click="sendToApp"></Button>
                                </div>
                            </Dialog>
                        </div>
                    </div>
