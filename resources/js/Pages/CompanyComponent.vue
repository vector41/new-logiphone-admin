        ...
        <div class="flex justify-start my-2 w-full overflow-auto rounded-sm bg-white overflow-y-auto">
            <table class="table margin-top w-fill">
                <tr>
                    ...
                    <td>
                        <Dropdown v-model="personality" :options="personalities" option-label="name"
                            placeholder="選択してください" class="w-fill !relative" />
                    </td>
                ...
                <tr v-show="is_company_branch == false">
                    ...
                    <td>
                        <div class="flex flex-row items-center gap-2">
                            <InputText class="w-28" input-class="!w-28" v-model="phone_1"
                                @keydown="checkPhone($event)" />
                            <span> - </span>
                            <InputText class="w-28" input-class="!w-28" v-model="phone_2"
                                @keydown="checkPhone($event)" />
                            <span> - </span>
                            <InputText class="w-28" input-class="!w-28" v-model="phone_3"
                                @keydown="checkPhone($event)" />
                        </div>
                    </td>
                </tr>
                        ...
                        <div class="flex flex-col gap-2">
                            <div class="flex flex-row items-center gap-2">
                                ...
                                <InputText class="w-28" input-class="!w-28" v-model="zip_2" placeholder="0005"
                                    @keydown="checkPhone($event)" />
                                <Button class="w-20 h-[34px] flex items-center justify-center text-white"
                                    @click="searchAddress">〒検索</Button>
                            </div>
                            <div class="flex flex-row items-center gap-2">
                                <Dropdown v-model="pref" :options="prefs" option-label="value" placeholder="都道府県を選択"
                                    class="w-56" />
                                <span> - </span>
                                <Dropdown v-model="city" :options="cities" option-label="value" placeholder="市区町村を選択"
                                    class="w-56" />
                            </div>
                            <div class="flex flex-row items-center gap-2">
                ...
                <tr>
                    ...
                    <td colspan="3">
                        <div class="flex gap-4">
                            <ImageUploadField v-for="img_item in imageObjs" :key="img_item.id" :field_id="img_item.id"
                                :img_file="img_item.file" :one_status="removable_status" @assign_file="assignFile"
                                @add_obj="addImgField" @remove_obj="removeImgField" />
                        </div>
                    </td>
                </tr>
            </table>
