                ...
                <tr v-if="props.type != 'update'">
                    ...
                    <td>
                        <div class="flex justify-start items-center gap-2 min-h-8">
                            <Button type="button" label="連絡先検索" size="small" class="text-white h-8"
                ...
                <tr>
                    ...
                    <td>
                        <div class="flex gap-3">
                            <div v-for="role_item of role_items" :key="role_item.id"
                                class="flex align-items-center gap-1">
                                <Checkbox v-model="roles" :inputId="role_item.id" name="category"
                                    :value="role_item.id" />
                ...
                <tr>
                    ...
                    <td colspan="3">
                        <div class="flex gap-4 min-h-[100px]">
                            <ImageUploadField v-for="img_item in cardImageObjs" :key="img_item.id"
                                :field_id="img_item.id" :one_status="removable_card_status"
                                @assign_file="assignCardFile" @add_obj="addCardImgField"
                                @remove_obj="removeCardImgField" />
                        </div>
                    </td>
                </tr>
                <tr>
                    ...
                    <td colspan="3">
                        <div class="flex gap-4 min-h-[100px]">
                            <ImageUploadField v-for="img_item in licenseImageObjs" :key="img_item.id"
                                :field_id="img_item.id" :one_status="removable_license_status"
                                @assign_file="assignLicensFile" @add_obj="addLicenseImgField"
                                @remove_obj="removeLicensImgField" />
                        </div>
                    </td>
                </tr>
            </table>
        </div>
        <Dialog v-model:visible="show_search_dialog" header="連絡先選択" :style="{ width: '1250px' }" position="center"
            ...
            <DataTable :value="lp_branches" paginator :paginatorPosition="both" :rows="15"
                ...
                <Column field="" header="" class="col-data">
                    <template #body="{ data }">
                        <Button type="button" class="employeer-dialog-btn" label="選択" size="small"
                            @click="selectCompany(data)"></Button>
                    </template>
                </Column>
